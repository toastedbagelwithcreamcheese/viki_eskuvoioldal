...
import MessageManager from '../../components/MessageManager'; // Új komponens importálása
...
async function RsvpAdminPage({ searchParams }) {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      ...
      <div className="mb-12"><ImageManager /></div>
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow mb-12">
        <table className="min-w-full text-left text-sm whitespace-nowrap">
          <thead className="border-b bg-gray-50"><tr><th className="px-6 py-4 font-semibold">Név</th><th className="px-6 py-4 font-semibold text-center">Transzfert kér?</th><th className="px-6 py-4 font-semibold text-center">Szállást kér?</th><th className="px-6 py-4 font-semibold">Személyes megjegyzés</th><th className="px-6 py-4 font-semibold">Visszajelzés ideje</th></tr></thead>
          <tbody>
            {rsvps.map((rsvp) => rsvp.is_attending ? (rsvp.guests.map(guest => (<tr key={guest.id} className="border-b hover:bg-gray-50"><td className="px-6 py-4 font-medium">{guest.name}</td><td className="px-6 py-4 text-center">{guest.needs_transfer ? '✅' : '❌'}</td><td className="px-6 py-4 text-center">{guest.needs_accommodation ? '✅' : '❌'}</td><td className="px-6 py-4">{guest.notes || '-'}</td><td className="px-6 py-4 text-gray-500">{new Date(rsvp.created_at).toLocaleString('hu-HU')}</td></tr>))) : (<tr key={rsvp.id} className="border-b bg-rose-50 hover:bg-rose-100"><td className="px-6 py-4 text-rose-700 italic" colSpan="4">{`Egy csoport jelezte, hogy nem tud eljönni.`}</td><td className="px-6 py-4 text-gray-500">{new Date(rsvp.created_at).toLocaleString('hu-HU')}</td></tr>))}
      ...
      {/* A régi üzenetlista helyett az új, interaktív komponenst használjuk */}
